...
def bootloader_installer(username: str = None):
    ...
    sub.run(["sudo", "mkdir", "-p", "/mnt/usr/share/procastios/wallpapers"])
    sub.run(["sudo", "cp", "/usr/local/bin/.procastios/assets/procasti/splash.png", "/mnt/usr/share/procastios/wallpapers/"])
    s(0.3)
...
def personalizer(username: str = None):
    ...
    if choose == 1:
        ...
        sub.run(["sudo", "arch-chroot", "/mnt", "pacman", "-S", "plasma-meta", "kde-applications", "--noconfirm"])
